...
export type SharedClinicalPayload = {
  ...
  treatment_detail_text_said?: string;
  room?: string;
  precaution?: string;
};
...
export function fillVisitInfo(h: PdfFormHelpers, data: SharedClinicalPayload) {
  ...
  h.setCheck("transport other check", !!tran && !knownTran.includes(tran));
  h.setText("transport other", !knownTran.includes(tran) ? tran : "");
...
export function fillPainSection(
  ...
  h.setCheck("Standard Precaution", precaution === "Standard Precaution");
  h.setCheck("Airborne", precaution === "Airborne");
  h.setCheck("Droplet", precaution === "Droplet");
  h.setCheck("drugResistance", precaution === "drug-Resistance");
  h.setCheck("Contact", precaution === "Contact");
  h.setCheck("Protective", precaution === "Protective");
  ...
  h.setCheck("Other_5", otherChar.length > 0);
  h.setText("after character other", otherChar.join(", "));
...
export function fillTreatmentItems(
  ...
  // บรรทัด 317 — เปลี่ยน key จาก hydrocallator เป็น Hydrocallator
  h.setCheck("Hydrocollator", ti.Hydrocallator?.checked ?? false);
  h.setText(
    "hydrocollator minute",
    ti.Hydrocallator?.fields?.["minutes"] ?? "",
  );
  h.setText("hydro area", ti.Hydrocallator?.fields?.["area"] ?? "");
  ...
  h.setText("Mode", ti.shockwave?.fields?.["area"] ?? ""); // ✅ เพิ่ม
  ...
  h.setText("after other describe", ti.othercheck?.fields?.["other"] ?? "");
}
