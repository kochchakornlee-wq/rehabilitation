...
// ── แก้ ASSESSMENT_TYPE_TH ────────────────────────────────────────────────────
const ASSESSMENT_TYPE_TH: Record<string, string> = {
  // before: ไม่มี checkbox ใน PDF TH → ข้ามไป
  before: "before surgery", // ← มีใน TH
  after: "after surgery",
...
// ── แก้ ASSESSMENT_TYPE_EN ────────────────────────────────────────────────────
const ASSESSMENT_TYPE_EN: Record<string, string> = {
  before: "before surgery", // ← มีใน EN
  after: "behind surgery", // ← EN ใช้ "behind" ไม่ใช่ "after"
...
export function fillKoosPdf(h: PdfFormHelpers, raw: unknown) {
  ...
  for (const fieldName of Object.values(typeMap)) {
    h.setCheck(fieldName, false);
  }

  const targetField = typeMap[assessType];
  if (targetField) {
    h.setCheck(targetField, true);
  }
