...
export async function POST(req: NextRequest) {
  ...
  for (const item of records) {
    ...
    await pool.query(
      ...
      [
        id,
        item.hn??null, item.datetime??null, item.topics??null, item.specify??null,
        item.education??null, item.readiness??null, item.barriers??null,
        item.methods??null, item.evaluation??null, item.provider??null,
      ]
    )
    ids.push(id)
  }
...
export async function PUT(req: NextRequest) {
  ...
  await pool.query(
    `UPDATE education_records SET
     ...
     WHERE id=?`,
    [
      body.datetime??null, body.topics??null, body.specify??null,
      body.education??null, body.readiness??null, body.barriers??null,
      body.methods??null, body.evaluation??null, body.provider??null,
      id,
    ]
  )
