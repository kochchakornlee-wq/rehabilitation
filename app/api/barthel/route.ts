  ...
  try {
    ...
    for (const row of rows) {
      ...
      await conn.query(
        ...
        [
          id,
          row.hn, row.session_number, row.assessed_at, row.assessed_by ?? null,
          row.feeding ?? null, row.transfers ?? null, row.grooming ?? null,
          row.toilet_use ?? null, row.bathing ?? null, row.mobility ?? null,
          row.stairs ?? null, row.dressing ?? null, row.bowels ?? null,
          row.bladder ?? null, row.total_score ?? null,
        ]
      )
    }
