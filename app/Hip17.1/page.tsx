...
export default function Hip17() {
  ...
  function buildHipPdfPayload() {
    ...
    const { bf, af, af1m } = allVisitData;

    const payload: Record<string, unknown> = {
      lang, // ← เพิ่ม
      PatientName: patientName, // ← P ใหญ่
      DOB: patientBirth,
  ...
  async function handlePreview() {
    console.log("hn:", hn);
    console.log("bf:", JSON.stringify(allVisitData.bf));
    console.log("allVisitData:", JSON.stringify(allVisitData));
    const payload = buildHipPdfPayload();
    console.log("VisitType ที่ส่ง:", payload.VisitType); // ← เพิ่ม
    console.log("lang:", payload.lang);
    console.log("payload:", JSON.stringify(payload, null, 2));
    console.log("bf:", allVisitData.bf); // ← เพิ่ม
    console.log("af:", allVisitData.af); // ← เพิ่ม
    console.log("payload keys:", Object.keys(payload)); // ← เพิ่ม

    setPdfPreview({ show: true, base64: null, loading: true, error: null });
    const result = await fetchPdfPreview("hip17", buildHipPdfPayload()); // ← เปลี่ยนเป็น hip17
    setPdfPreview({
      show: true,
  ...
  useEffect(() => {
    ...
    const fetchAllVisits = async () => {
      const [bf, af, af1m] = await Promise.all([
        fetch(`/api/hip17?hn=${hn}&visit_type=ก่อนผ่าตัด`).then((r) =>
          r.json(),
        ),
        fetch(`/api/hip17?hn=${hn}&visit_type=หลังผ่าตัด`).then((r) =>
          r.json(),
        ),
        fetch(`/api/hip17?hn=${hn}&visit_type=หลังผ่าตัด 1 เดือน`).then((r) =>
          r.json(),
        ),
      ]);
